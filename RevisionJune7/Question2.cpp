...
int generatePermutations(int n , vector<int> &v , int level) {
    ...
    for (int i = 0; i < n; i++) {
        // level decides where you are inside the permutation , I decides which element is being considered
        // thus for that reason we would be needing to check inside the level
        int pos = level + 1;
